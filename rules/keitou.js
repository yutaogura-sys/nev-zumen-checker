...
(function (root) {
  ...
  const rule = {
    ...
    prompt: {
      ...
      guidance: '## この図面の要点（ブレーカー判定を特に厳格に）\n- 図面名称は正確に「電気系統図」。「配線系統図」「電気配線図」は不可。\n- **ブレーカーは個別に容量(AF/AT)が必須**：シンボルだけで容量数値が無い、AF/ATの片方のみ、はfail。主幹と分岐は別個に評価。\n- メーカー名・型式は表題欄・機器シンボル付近・凡例・機器リスト・注記欄・2ページ目以降まで全領域を探索してから判定する。\n- デマンド制御：デマンドコントローラー等を使用している場合、⑩の記載（機能注記＋同時稼働台数）が必須。総設置台数が主幹ATの定格動作台数（40AT→1台/75AT→2台/100AT→3台/125AT→4台/150AT→5台/200AT→6台/225AT→7台/250AT→8台）を超える場合は、そもそも容量がR7補正要件（全数フル出力の同時稼働前提）を満たさない可能性→warn。\n- 接地種別の大文字小文字（Ec/EC/Ed/ED）は不問。\n\n## ★複数基設置の容量評価（最重要・R7補正で基準変更）\n- **R7補正の手引き（事業要件）により、複数基設置では「設置するすべての充電設備の承認された定格出力での同時稼働」を前提に、必要な電気配線・ブレーカー等を設置することが要件**。\n- したがって幹線ケーブル・主幹ブレーカー・接地線の台数別スペック（man_k_cable_count / man_k_main_breaker / man_k_ground の仕様表）は、**必ず「総設置台数」に対して評価すること**。\n- **デマンドコントロール／ロードバランシング（LB）／出力制御の実装は認められているが、設備容量を減らす根拠にはならない**。LB注記があっても、総設置台数のフル出力に不足する幹線・主幹は warn（容量がR7補正要件を満たさない可能性）とし、LBを理由に妥当と判定してはならない（旧年度のLB緩和基準は廃止）。\n- デマンドコントローラー等を使用している図面では、⑩デマンド制御の記載（機能の注記＋同時稼働台数）と制御装置の仕様（メーカー名・型式）を確認する。\n- 同時運転台数（記載があれば）を detected_info.simultaneous_count に、総設置台数を charger_count に、それぞれ読み取って記載すること（同時運転台数は記載チェック用であり、容量評価には使わない）。\n- 条件に当てはまらない条件付き項目はnaとする。\n\n## 作図センター基準（man_*）の扱い（2026-07-17基準変更）\n- man_* の各項目はR6作図マニュアル由来の**旧基準**（最新版マニュアルは未作成）。判定は行うが、現行案件の不合格断定には用いない＝**逸脱してもfailではなくwarn（要確認）で報告**し、detailに逸脱内容を具体的に書くこと。\n\n## 正解事例の系統フロー（旧ツール実績の参考パターン）\n- 目的地充電: 図面左側に既設キュービクル(黒)→幹線(赤)→EV用分電盤(赤)→分岐ブレーカー(赤)→充電設備(赤) の流れで描かれることが多い。\n- 基礎充電: 責任分界点→電力量計→分電盤→制御盤→充電設備 の流れで描かれることが多い。\n- fail/warn の項目では、detail に該当箇所のページ番号とおおよその位置（例:「P1 右下表題欄付近」「P2 機器リスト」）を含めること。記載が見当たらない場合は「該当記載なし」と明記する。',
    },
    categories: {
      // NeV要件判定
    ...
    checks: [
      ...
      { id: 'nev_location', group: 'nev', category: 'nev_basic_info', label: '設置場所名称の記載', required: true, critical: true,
        description: '設置場所名称が記載され、設置場所そのものが確認できること。※申請書で入力した名称との一致（略称不可）は本ツールでは照合できないため判定対象外（記載有無の判定と読取名称の提示のみ）。読み取った名称をdetailに必ず記載し、申請書との一致は人手で確認すること。' },
      ...
      { id: 'nev_charger_type', group: 'nev', category: 'nev_charger_spec', label: '充電設備の種類の記載（任意）', required: false, src: '記入例',
        description: '急速・普通等の種類が記載されていること。※手引き5-9-4本文の要求はメーカー名・型式のみ（記入例の図中ラベル由来）のため任意（2026-07-14格下げ・ユーザー承認）。' },
      ...
      { id: 'nev_demand', group: 'nev', category: 'nev_demand', label: 'デマンド制御の記載', required: false, condition: 'デマンドコントローラー等を使用している場合',
        description: 'デマンドコントローラー・課金デバイス等を使用している場合、デマンド制御の記載（機能の注記＋同時稼働台数）が必須（記入例⑩）。使用していない場合はna。※R7補正の手引き（事業要件）により、電気設備の容量は総設置台数の承認定格出力での同時稼働を前提に担保が必要——デマンド制御の実装は認められるが、容量不足の代替にはならない（総設置台数が主幹ATの定格動作台数=40AT→1台〜250AT→8台を超える場合はwarn）。' },
      ...
      { id: 'nev_new_contract', group: 'nev', category: 'nev_power_source', label: '新規契約時の引込開閉器等のメーカー名・型式', required: false, condition: '特別措置など新規で契約する場合（新設分電盤・引込開閉器・新設キュービクル等）',
        description: '特別措置など新規で契約する場合、引込開閉器等（新設分電盤・新設キュービクル含む）を図示し、そのメーカー名と型式が記載されていること（手引き5-9-4）。シンボル付近・凡例・機器リスト・注記欄・2ページ目以降も含め全領域を確認すること。' },
      ...
      { id: 'man_m_date', group: 'manual', category: 'man_basic', label: '作成日の記載（指定日との一致は人手確認）', required: true,
        description: '作成日が記載されていること（記載があればpass。読み取った日付をdetailに必ず記載）。※発注元指定日との一致は本ツールでは照合できないため判定対象外＝人手確認欄で確認すること。' },
      ...
      { id: 'man_m_panel_name', group: 'manual', category: 'man_panel', label: '盤名称の記載（配線ルート図との一致は人手確認）', required: true,
        description: '電源盤・分電盤の名称が記載されていること（記載があればpass。読み取った盤名称をdetailに必ず記載）。※配線ルート図との一致は本ツールには当該図面が渡らないため判定不能＝判定対象外。案件まとめチェックの図面間照合または人手で確認すること。' },
      ...
      { id: 'man_k_date', group: 'manual', category: 'man_basic', label: '作成日の記載（指定日との一致は人手確認）', required: true,
        description: '作成日が記載されていること（記載があればpass。読み取った日付をdetailに必ず記載）。※発注元指定日との一致は本ツールでは照合できないため判定対象外＝人手確認欄で確認すること。' },
      ...
      { id: 'man_k_power_type', group: 'manual', category: 'man_equip', label: '配電方法の記載（例：1Φ3W100/200V）', required: true,
        description: '受電方式が記載されていること。' },
      { id: 'man_k_panel_name', group: 'manual', category: 'man_panel', label: '盤名称（制御盤含む）の記載（配線ルート図との一致は人手確認）', required: true,
        description: '電源盤・分電盤・制御盤の名称が記載されていること（記載があればpass。読み取った盤名称をdetailに必ず記載）。※配線ルート図との一致は本ツールには当該図面が渡らないため判定不能＝判定対象外。案件まとめチェックの図面間照合または人手で確認すること。' },
    ...
    deterministic: [
      ...
      { fn: 'demand_rated_count',
        ...
        requires: {
          ...
          simultaneous_count: 'デマンド/LB使用時に図面記載の同時稼働台数を数値で（記載チェック用。容量評価は総設置台数で行う）。記載がなければ空',
        } },
